...
struct
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
	  ...
	  (* find E *)
	  let rec findE = function
		| (_, Loc l) :: t ->
			if (List.mem l !reachable_locs) then findE t
			else
	  ...
	  let rec findR = function
	  	| (_, l) :: t ->
			if (List.mem_assoc l m) then
				let _ = (match (List.assoc l m) with
				| R r -> findR r
				| _ -> ()
				) in
				if (List.mem l !reachable_locs) then findR t
				else
					let _ = reachable_locs := l :: (!reachable_locs) in
					findR t
			else ()
	  ...
	  in
	  (* find S *)
	  let rec findS = function
		| (M (_, Proc (_, _, e))) :: t ->
			let _ = findE e in
			findS t
		| (M (_, Loc l)) :: t ->
			if (List.mem l !reachable_locs) then findS t
			else
				let _ = reachable_locs := l :: (!reachable_locs) in
				findS t
		| (V (R r)) :: t ->
