...
struct
  ...
  let op2fn =
    ...
    | EQ ->
      (fun (v1,v2) ->
		...
		| (Pair _, Pair _) -> raise (TypeError "Cannot compare PAIR type")
		| (Closure _, Closure _) -> raise (TypeError "Cannot compare CLOSURE type")
		| _ -> raise (TypeError "Cannot compare two different types")
	  )
  ...
  let rec eval env mem exp = 
    ...
    | APP (e1, e2) ->
      ...
      | RecFun (f, x, e) ->
	    eval ((env' @+ (x, v2)) @+ (f, Closure (RecFun (f, x, e), env'))) m'' e
	  )
    | LET (d, e) ->
	  (match d with
	  | VAL (x, expr) ->
	    let (v1, m') = eval env mem expr in
		eval (env @+ (x, v1)) m' e
	  | REC (f, x, expr) ->
	  (*
	    let (fn, m') = eval env mem expr in
		let (c, env') = getClosure fn in
		(match c with
		| Fun (arg, ex) ->
		  eval (env @+ (f, Closure (RecFun (f, x, ex), env'))) m' e
		| _ -> raise (TypeError "function binding failure")
		)
	  *)
	    eval (env @+ (f, Closure (RecFun (f, x, expr), env))) mem e
	  )
	| IF (e1, e2, e3) ->
      let (v1, m') = eval env mem e1 in
