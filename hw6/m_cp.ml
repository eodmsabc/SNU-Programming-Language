...
struct
  ...
  let rec eval env mem exp = 
	...
	| LET (d, e) -> failwith "Unimplemented" (* TODO *)
    | IF (e1, e2, e3) ->
    ...
    | WRITE e ->
      ...
      (v, m')
	| MALLOC e -> failwith "Unimplemented" (* TODO *)
	| ASSIGN (e1, e2) -> failwith "Unimplemented" (* TODO *)
	| BANG e -> failwith "Unimplemented" (* TODO *)
	| SEQ (e1, e2) -> failwith "Unimplemented" (* TODO *)
    | PAIR (e1, e2) -> 
      let (v1, m') = eval env mem e1 in
      let (v2, m'') = eval env m' e2 in
      (Pair (v1, v2), m'')
